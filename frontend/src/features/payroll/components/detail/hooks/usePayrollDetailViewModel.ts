...
export function usePayrollDetailViewModel({
  ...
  const enrichedPayouts = useMemo(() => {
    ...
    return rows.map((row) => {
      ...
      const leaveCountInPeriod = Number(row.leave_count_in_period ?? 0)
      const educationLeaveCountInPeriod = Number(row.education_leave_count_in_period ?? 0)
