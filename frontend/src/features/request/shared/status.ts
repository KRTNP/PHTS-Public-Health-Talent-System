...
export function getRequestPendingStepLabel(step?: number | null) {
  switch (step) {
    case 1:
...
export function getRequestStatusLabel(status: string, step?: number | null) {
  switch (status) {
    case "DRAFT":
      return "ฉบับร่าง"
    case "PENDING":
      return getRequestPendingStepLabel(step)
    case "APPROVED":
      return "อนุมัติแล้ว"
    case "REJECTED":
      return "ไม่อนุมัติ"
