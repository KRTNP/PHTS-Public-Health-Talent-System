...
type HeadScopeDbRole =
  | typeof DB_HEAD_SCOPE_ROLE_WARD
  | typeof DB_HEAD_SCOPE_ROLE_DEPT;
...
export const DB_HEAD_SCOPE_ROLE_SQL_LIST = DB_HEAD_SCOPE_ROLES.map(
  (role) => `'${role}'`,
).join(", ");
export const DB_HEAD_SCOPE_ROLE_SQL_ORDER = DB_HEAD_SCOPE_ROLE_ORDER.map(
  (role) => `'${role}'`,
).join(", ");
...
export function toHeadScopeCategory(
  role: string | null | undefined,
): HeadScopeCategory | null {
...
export function toHeadScopeCategoryList(
...
export function isHeadScopeCategory(
  role: string | null | undefined,
): role is HeadScopeCategory {
