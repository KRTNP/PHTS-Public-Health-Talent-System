...
jest.mock(
  "@/modules/workforce-compliance/services/license-compliance.digest.service.js",
  () => ({
...
jest.mock(
  "@/modules/notification/services/notification-outbox.service.js",
  () => ({
...
describe("scheduled jobs runner", () => {
  ...
  test("selectJobs returns defaults when args are empty", async () => {
    const { selectJobs, JOBS } =
      await import("@/scripts/ops/runners/scheduled-jobs.js");
    expect(selectJobs([])).toEqual(JOBS);
  });

  test("backup job returns skipped summary when not due", async () => {
    const { shouldRunScheduledBackup, runBackupJob } =
  ...
  test("license-auto-cut job returns success summary", async () => {
    const { runLicenseAutoCutRestore } =
      await import("@/modules/workforce-compliance/services/jobs/index.js");
    (runLicenseAutoCutRestore as jest.Mock).mockResolvedValue({
      cut: 2,
      restored: 1,
    });
