...
const ASSISTANT_TOKENS = ['รองหัวหน้า', 'ผู้ช่วยหัวหน้า', 'ผู้ช่วยหัวหน้าตึก', 'รองหัวหน้าตึก'];
...
function isDirector(position: string): boolean {
  return position.includes('ผู้อำนวยการ') && !position.includes('รองผู้อำนวยการ');
}
...
function deriveRole(row: HrUserRow): string {
  ...
  if (isDirector(positionName) || isDirector(specialPosition)) return 'DIRECTOR';

  // 2. Finance roles (ไม่รวมกลุ่มงานบัญชี)
  const isAccountingUnit = specialPosition.includes('บัญชี');
  if (department === 'กลุ่มงานการเงิน' && !isAccountingUnit && isHeadDept(specialPosition)) {
    return 'HEAD_FINANCE';
  }
  if (department === 'กลุ่มงานการเงิน' && !isAccountingUnit) return 'FINANCE_OFFICER';
