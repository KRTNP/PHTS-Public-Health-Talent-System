 ...
 */
export const getCycles = asyncHandler(
  async (req: Request, res: Response<ApiResponse>): Promise<void> => {
    ...
    res.json({
      success: true,
      data: cycles.map((cycle) => mapCycleResponse(cycle)),
    });
  },
);
...
export const createCycle = asyncHandler(
  async (_req: Request, res: Response<ApiResponse>): Promise<void> => {
...
export const getQueue = asyncHandler(
  async (req: Request, res: Response<ApiResponse>): Promise<void> => {
    ...
    const page = asString(query.page)
      ? Number.parseInt(String(query.page), 10)
      : 1;
    const limit = asString(query.limit)
      ? Number.parseInt(String(query.limit), 10)
      : 20;
    const batchId = asString(query.batch_id)
      ? Number.parseInt(String(query.batch_id), 10)
      : undefined;
    const status = asString(
      query.status,
    ) as accessReviewService.AccessReviewQueueListInput["status"];
    const reasonCode = asString(query.reason_code);
    const currentRole = asString(query.current_role);
    const isActive =
      query.is_active !== undefined ? Number(query.is_active) : undefined;
 ...
 */
export const getQueueEvents = asyncHandler(
  async (req: Request, res: Response<ApiResponse>): Promise<void> => {
    const queueId = Number.parseInt(req.params.id, 10);
    const limit = req.query.limit
      ? Number.parseInt(req.query.limit as string, 10)
      : 100;
    const events = await accessReviewService.getAccessReviewQueueEvents(
      queueId,
      limit,
    );
 ...
 */
export const resolveQueueItem = asyncHandler(
  async (req: Request, res: Response<ApiResponse>): Promise<void> => {
    const queueId = Number.parseInt(req.params.id, 10);
    const user = req.user;
    if (!user) {
 ...
 */
export const bulkResolveQueueItems = asyncHandler(
  async (req: Request, res: Response<ApiResponse>): Promise<void> => {
    const user = req.user;
    if (!user) {
      throw new AuthenticationError("Unauthorized access");
 ...
 */
export const completeCycle = asyncHandler(
  async (req: Request, res: Response<ApiResponse>): Promise<void> => {
    const cycleId = Number.parseInt(req.params.id, 10);
    const user = req.user;
    if (!user) {
 ...
 */
export const autoReviewCycle = asyncHandler(
  async (req: Request, res: Response<ApiResponse>): Promise<void> => {
    const cycleId = Number.parseInt(req.params.id, 10);
    const user = req.user;
    if (!user) {
    ...
    const result = await accessReviewService.autoReviewCycle(
      cycleId,
      reviewerId,
      {
