...
jest.mock(
  "@/modules/identity/repositories/identity-role-policy.repository.js",
  () => ({
    __esModule: true,
...
jest.mock(
  "@/modules/access-review/repositories/access-review.repository.js",
  () => ({
    __esModule: true,
...
describe("access review role mismatch policy", () => {
  ...
  test("does not flag role mismatch when derived head role has no active scope map", async () => {
    ...
    const { refreshReviewCycleFromSync } =
      await import("@/modules/access-review/services/access-review.service.js");
