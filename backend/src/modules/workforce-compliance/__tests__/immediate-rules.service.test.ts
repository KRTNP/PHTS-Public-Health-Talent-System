...
describe("applyImmediateMovementEligibilityCutoff", () => {
  ...
  test("formats cutoff date using ops timezone instead of UTC slicing", async () => {
    (
      WorkforceComplianceRepository.getMovementOutCandidates as jest.Mock
    ).mockResolvedValue([
      {
    ...
    (
      WorkforceComplianceRepository.setEligibilityExpiry as jest.Mock
    ).mockResolvedValue(1);

    const result = await applyImmediateMovementEligibilityCutoff();

    expect(
      WorkforceComplianceRepository.setEligibilityExpiry,
    ).toHaveBeenCalledWith("1234567890123", "2026-03-01", undefined);
