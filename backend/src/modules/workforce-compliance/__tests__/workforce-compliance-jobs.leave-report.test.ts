...
describe("runLeaveReportAlerts", () => {
  ...
  test("uses policy windows and sends notification for matched records", async () => {
    ...
    expect(
      WorkforceComplianceRepository.getLeaveReportCandidates,
    ).toHaveBeenNthCalledWith(1, ["ordain"], 14, expect.any(Date));
    expect(
      WorkforceComplianceRepository.getLeaveReportCandidates,
    ).toHaveBeenNthCalledWith(2, ["military"], 15, expect.any(Date));
