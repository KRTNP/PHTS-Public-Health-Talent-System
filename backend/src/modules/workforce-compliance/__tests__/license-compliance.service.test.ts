...
jest.mock(
  "@/modules/workforce-compliance/repositories/alert-logs.repository.js",
  () => ({
    AlertLogsRepository: {
...
jest.mock(
  "@/modules/workforce-compliance/repositories/workforce-compliance.repository.js",
  () => ({
...
jest.mock(
  "@/modules/workforce-compliance/repositories/license-compliance.repository.js",
  () => ({
...
import { notifyLicenseCompliance } from "@/modules/workforce-compliance/services/license-compliance.service.js";
...
describe("notifyLicenseCompliance", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  test("uses ops timezone when building daily dedupe key", async () => {
    ...
    const result = await notifyLicenseCompliance(
    ...
    const hasPayloadHashArg = (AlertLogsRepository.hasPayloadHash as jest.Mock)
      .mock.calls[0]?.[0];
