...
export async function updateSLAConfig(
  ...
  try {
    ...
    if (!slaDays || slaDays < 1) {
      res.status(400).json({ success: false, error: 'slaDays must be at least 1' });
      return;
    }

    await slaService.updateSLAConfig(stepNo, slaDays, reminderBeforeDays, reminderAfterDays);
...
export async function calculateBusinessDays(
  ...
  try {
    ...
    const businessDays = await slaService.calculateBusinessDays(startDate, endDate);
