...
const ensureReportablePeriod = (params: {
  ...
  const snapshotReady =
    String(params.snapshotStatus ?? "").toUpperCase() === "READY";
...
export async function markPayoutAsPaid(
  ...
  try {
    ...
    const payout =
      await FinanceRepository.findPayoutWorkflowContextByIdForUpdate(
        payoutId,
        conn,
      );
...
export async function getPayoutsByPeriod(
  ...
  const period =
    await FinanceRepository.findPeriodWorkflowContextById(periodId);
  if (!period) {
  ...
  return FinanceRepository.findPayoutsByPeriod(
    periodId,
    status,
    search,
    professionCode,
