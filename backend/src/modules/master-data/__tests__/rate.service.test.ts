...
jest.mock(
  "@/modules/master-data/repositories/master-data.repository.js",
  () => ({
...
describe("MasterData Rate Service", () => {
  ...
  test("deleteMasterRate permanently deletes rate when no references and writes audit log", async () => {
    ...
    (
      MasterDataRepository.countMasterRateReferences as jest.Mock
    ).mockResolvedValueOnce(0);
    (
      MasterDataRepository.deleteMasterRatePermanently as jest.Mock
    ).mockResolvedValueOnce(undefined);

    await deleteMasterRate(rateId, actorId);

    expect(MasterDataRepository.countMasterRateReferences).toHaveBeenCalledWith(
      rateId,
    );
    expect(
      MasterDataRepository.deleteMasterRatePermanently,
    ).toHaveBeenCalledWith(rateId);
  ...
  test("getRateHierarchy preserves rate ids on criteria and subCriteria", async () => {
    (
      MasterDataRepository.findRateHierarchyRows as jest.Mock
