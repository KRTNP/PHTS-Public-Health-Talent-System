...
describe("OpsJobRunService", () => {
  ...
  test("records successful run with summary", async () => {
    const { OpsJobRunsRepository } =
      await import("@/modules/system/repositories/ops-job-runs.repository.js");
    const { OpsJobRunService } =
      await import("@/modules/system/services/ops-job-run.service.js");
