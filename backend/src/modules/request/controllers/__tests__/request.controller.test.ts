...
import {
  ...
  makeStatusJsonRes,
  makeUser,
} from "./request.controller.test-helpers.js";
...
describe("request.controller", () => {
  ...
  describe("createRequest on behalf", () => {
    it("allows PTS_OFFICER to create request for selected personnel even when actor is not in emp_profiles", async () => {
      ...
      (requestRepository.findUserCitizenId as jest.Mock).mockResolvedValue(
        "1100702579863",
      );
      (requestRepository.findEmployeeExists as jest.Mock).mockResolvedValue(
        true,
      );
  ...
  describe("persistManualOcrPrecheck", () => {
    it("passes manual OCR payload through", async () => {
      ...
      (
        requestCommandService.persistManualOcrPrecheck as jest.Mock
      ).mockResolvedValue({
        saved: true,
      });
      ...
      expect(
        requestCommandService.persistManualOcrPrecheck,
  ...
  describe("persistEligibilityManualOcrPrecheck", () => {
    it("passes manual OCR payload through for eligibility detail", async () => {
      ...
      (
        requestCommandService.persistEligibilityManualOcrPrecheck as jest.Mock
      ).mockResolvedValue({
        saved: true,
      });

      const next = makeNext();
      await (requestController.persistEligibilityManualOcrPrecheck as any)(
        req,
        res,
        next,
      );
      ...
      expect(
        requestCommandService.persistEligibilityManualOcrPrecheck,
      ).toHaveBeenCalledWith(
        2978,
        46941,
        "PTS_OFFICER",
    ...
    it("runs allowance attachment OCR via backend", async () => {
      ...
      (
        requestCommandService.runEligibilityAttachmentsOcr as jest.Mock
      ).mockResolvedValue({
        saved: true,
        count: 2,
        success_count: 2,
      ...
      const next = makeNext();
      await (requestController.runEligibilityAttachmentsOcr as any)(
        req,
        res,
        next,
      );
      ...
      expect(
        requestCommandService.runEligibilityAttachmentsOcr,
      ).toHaveBeenCalledWith(2978, 46941, "PTS_OFFICER", {
        attachments: [
          { attachment_id: 11, source: "eligibility" },
          { attachment_id: 22, source: "request" },
        ],
  ...
  describe("getOcrPrecheckHistory", () => {
    it("passes OCR history filters through and returns paged payload", async () => {
      ...
      (
        requestQueryService.getOcrPrecheckHistory as jest.Mock
      ).mockResolvedValue({
        items: [
          {
            request_id: 67903,
  ...
  describe("getEligibilitySummary", () => {
    it("passes summary filters through and returns alert summary payload", async () => {
      ...
      (
        requestQueryService.getEligibilitySummary as jest.Mock
      ).mockResolvedValue({
  ...
  describe("updateVerificationChecks", () => {
    it("calls command service and returns updated request payload", async () => {
      ...
      (
        requestCommandService.updateVerificationChecks as jest.Mock
      ).mockResolvedValue({
        request_id: 42,
        status: "PENDING",
      });
      ...
      expect(
        requestCommandService.updateVerificationChecks,
      ).toHaveBeenCalledWith(42, 10, "PTS_OFFICER", req.body);
      expect(res.json).toHaveBeenCalledWith({
        success: true,
  ...
  describe("action endpoint parity", () => {
    it("dispatches unified APPROVE action to approval service", async () => {
