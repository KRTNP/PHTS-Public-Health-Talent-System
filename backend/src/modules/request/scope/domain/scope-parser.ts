...
const WARD_SCOPE_PATTERNS: Array<{ label: string; regex: RegExp }> = [
  {
    ...
    regex: /^หัวหน้าตึก\/หัวหน้างาน\s*[-:–—]?\s*/u,
  },
...
export function buildScopesFromSpecialPosition(
  ...
  const normalizeDeptScopeName = (
    ...
    if (
      prefix === "หัวหน้ากลุ่มงาน" &&
      !name.includes("กลุ่มงาน") &&
      !name.includes("ภารกิจ")
    ) {
  ...
  for (const scope of allScopes) {
    ...
    if (EXCLUDED_SCOPE_TOKENS.some((token) => normalizedScope.includes(token)))
      continue;
    ...
    if (deptScope?.scopeName) {
      addScope(
        deptScopes,
        normalizeDeptScopeName(deptScope.scopeName, deptScope.prefix),
      );
      continue;
    }
  }
