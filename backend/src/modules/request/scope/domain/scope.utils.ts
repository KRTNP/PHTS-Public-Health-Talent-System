 ...
 *
 * Parses special_position to determine WARD_SCOPE/DEPT_SCOPE
 ...
 *
 * @param wardScopes - WARD_SCOPE scopes (with overlaps already removed)
 * @param deptScopes - DEPT_SCOPE scopes
 * @param requestDept - request.department
 * @param requestSubDept - request.sub_department
 * @returns 'WARD_SCOPE' | 'DEPT_SCOPE' | 'NONE'
 */
export function resolveApproverRole(
  ...
  // 1. If both match by unit scope -> WARD_SCOPE wins
  // 2. If WARD_SCOPE matches unit and DEPT_SCOPE matches dept -> WARD_SCOPE wins (more specific)
  // 3. If both match by dept scope -> DEPT_SCOPE wins
