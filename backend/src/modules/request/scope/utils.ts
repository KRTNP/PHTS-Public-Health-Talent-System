...
export function inferScopeType(scopeName: string): ScopeType {
  ...
  if (name.includes('หัวหน้าพยาบาล') || name.includes('รองผู้อำนวยการฝ่ายการพยาบาล')) {
    return 'IGNORE';
  }
  ...
  // Check UNIT keywords
  if (name.includes('งาน') || name.includes('หอ') || name.includes('หน่วย') || name.includes('ศูนย์')) {
    return 'UNIT';
  }
 ...
 */
export function parseSpecialPositionScopes(specialPosition: string | null): string[] {
 ...
 */
export function removeOverlaps(wardScopes: string[], deptScopes: string[]): string[] {
 ...
 */
export function scopeMatches(scope: string, target: string | null | undefined): boolean {
 ...
 */
export function anyScopeMatches(scopes: string[], target: string | null | undefined): boolean {
  if (!target) return false;
