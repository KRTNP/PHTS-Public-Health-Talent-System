...
describe("RequestQueryService query methods", () => {
  ...
  describe("getRequestDetails OCR precheck", () => {
    it("hydrates OCR precheck from dedicated OCR table when submission_data has no OCR payload", async () => {
      ...
      jest
        ...
        .mockResolvedValue([]);
      jest
        .spyOn(requestRepository, "findLatestVerificationSnapshotByRequestId")
        .mockResolvedValue(null);
      jest
        .spyOn(requestRepository, "findLatestEligibilityByRequestId")
        .mockResolvedValue({
          eligibility_id: 3363,
      ...
      expect(OcrRequestRepository.findRequestPrecheck).toHaveBeenCalledWith(
        2001,
      );
  ...
  describe("getApprovalHistory", () => {
    it("returns requester profile and approval actions for history rows", async () => {
      jest
        .spyOn(requestRepository, "findUserIdsByRole")
      ...
      jest
        ...
        .mockResolvedValue([
          {
            request_id: 1001,
            last_action_date: new Date("2026-02-17T12:00:00.000Z"),
          },
        ]);
      ...
      const result = await requestQueryService.getApprovalHistory(
        12,
        "HEAD_HR",
      );
  ...
  describe("getOcrPrecheckHistory", () => {
    it("returns OCR history items with document metadata", async () => {
      jest
        .spyOn(OcrRequestRepository, "findRequestPrecheckHistory")
        .mockResolvedValue({
          items: [
            {
              request_id: 67909,
  ...
  describe("getRequestById read access", () => {
    ...
    it("allows PTS_OFFICER to read pending request regardless of assigned officer", async () => {
      ...
      jest
        .spyOn(requestQueryService, "getRequestDetails")
        .mockResolvedValue({ request_id: 67919 } as any);

      await expect(
        requestQueryService.getRequestById(67919, 49000, "PTS_OFFICER"),
      ).resolves.toMatchObject({ request_id: 67919 });
    });
  });
});
