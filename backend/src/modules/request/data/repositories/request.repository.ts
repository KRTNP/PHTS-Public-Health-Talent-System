...
import {
  buildEligibilityAlertWhere,
  type EligibilityAlertFilter,
} from "@/modules/request/data/repositories/eligibility-alert-filter.js";
import {
...
export class RequestRepository {
  ...
  async findEligibilityList(
    ...
    const [rows] = await db.query<RowDataPacket[]>(
      ...
      SELECT
        ...
        ep.email,
        ep.phone,
        COALESCE(elig_dup.active_eligibility_count, 0) AS active_eligibility_count,
        upcoming_change.upcoming_change_type,
        upcoming_change.upcoming_change_effective_date,
        lic.latest_license_status,
        lic.latest_license_valid_from,
        lic.latest_license_valid_until,
        r.profession_code,
        r.group_no,
        r.item_no,
        r.sub_item_no,
  ...
  private buildEligibilityWhere(filters: {
    ...
    const licenseFilter = buildEligibilityLicenseStatusWhere(
      filters.licenseStatus ?? null,
      expiringDays,
    );
    if (licenseFilter.clause) {
      where.push(licenseFilter.clause);
  ...
  async findEligibilityListMeta(
    ...
    const [rows] = await db.query<RowDataPacket[]>(
      ...
      SELECT
        ...
        MAX(CASE WHEN r.sub_item_no IS NOT NULL AND TRIM(r.sub_item_no) <> '' THEN 1 ELSE 0 END) AS has_sub_item_no
      FROM req_eligibility e
      LEFT JOIN emp_profiles ep ON ep.citizen_id = e.citizen_id
      ${buildActiveEligibilityCountJoinSql()}
      ${buildUpcomingPersonnelChangeJoinSql()}
  ...
  async findEligibilitySummary(
    ...
    const [rows] = await db.query<RowDataPacket[]>(
      ...
      SELECT
        ...
        COUNT(DISTINCT CASE WHEN lic.citizen_id IS NULL THEN e.citizen_id ELSE NULL END) AS no_license_people,
        COUNT(DISTINCT CASE WHEN COALESCE(elig_dup.active_eligibility_count, 0) > 1 THEN e.citizen_id ELSE NULL END) AS duplicate_people,
        COUNT(DISTINCT CASE WHEN upcoming_change.upcoming_change_type IS NOT NULL THEN e.citizen_id ELSE NULL END) AS upcoming_change_people,
        MAX(e.created_at) AS updated_at
      FROM req_eligibility e
      LEFT JOIN emp_profiles ep ON ep.citizen_id = e.citizen_id
      ${buildActiveEligibilityCountJoinSql()}
      ...
      `,
      [filters.expiringDays ?? ELIGIBILITY_EXPIRING_DAYS, ...params],
    );
    return rows;
  }
  ...
  async findLatestEligibilityByRequestId(
    requestId: number,
    connection?: PoolConnection,
  ): Promise<RowDataPacket | null> {
    const db = this.getDb(connection);
    const [rows] = await db.query<RowDataPacket[]>(
      ...
      LEFT JOIN cfg_payment_rates r ON r.rate_id = e.master_rate_id
      WHERE request_id = ?
      ORDER BY e.is_active DESC, e.eligibility_id DESC
      LIMIT 1
      `,
      [requestId],
    );
    ...
    const sql = `
      SELECT
        ...
        (
          SELECT l.license_no
          FROM emp_licenses l
          WHERE l.citizen_id = r.citizen_id
          ORDER BY
            l.valid_until DESC,
            l.valid_from DESC,
            l.license_id DESC
  ...
  async findByUserId(userId: number): Promise<RequestSubmissionEntity[]> {
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT r.*, u.citizen_id, u.role
       ...
       WHERE r.user_id = ?
          OR CAST(JSON_UNQUOTE(JSON_EXTRACT(r.submission_data, '$.created_by_officer_id')) AS UNSIGNED) = ?
  ...
  async getScopeMappings(
    ...
    const db = this.getDb(connection);
    const dbRole = toDbHeadScopeRole(role as "WARD_SCOPE" | "DEPT_SCOPE");
    const [rows] = await db.query<RowDataPacket[]>(
    ...
    const [rows] = await db.query<RowDataPacket[]>(
      `SELECT DISTINCT role
       ...
       WHERE is_active = 1
         ...
         AND (user_id = ? OR (? IS NOT NULL AND citizen_id = ?))
       ORDER BY FIELD(role, ${DB_HEAD_SCOPE_ROLE_SQL_ORDER})`,
      [userId, citizenId ?? null, citizenId ?? null],
    );
    return rows
  ...
  async findScopeRolesForCitizensInScope(
    scopeType: "UNIT" | "DEPT",
    scopeName: string,
    citizenIds: string[],
    connection?: PoolConnection,
  ): Promise<Array<{ citizen_id: string; role: "WARD_SCOPE" | "DEPT_SCOPE" }>> {
    ...
    const [rows] = await db.query<RowDataPacket[]>(
      `
        SELECT
          COALESCE(NULLIF(TRIM(m.citizen_id), ''), u.citizen_id) AS citizen_id,
          m.role
        FROM special_position_scope_map m
        LEFT JOIN users u ON u.id = m.user_id
        WHERE m.is_active = 1
          AND m.scope_type = ?
          AND LOWER(TRIM(m.scope_name)) = LOWER(TRIM(?))
          AND COALESCE(NULLIF(TRIM(m.citizen_id), ''), u.citizen_id) IN (?)
          AND m.role IN ('WARD_SCOPE', 'DEPT_SCOPE')
        ORDER BY FIELD(m.role, 'DEPT_SCOPE', 'WARD_SCOPE')
      `,
  ...
  async findActiveScopeMappingsByScope(
    scopeType: "UNIT" | "DEPT",
    scopeName: string,
    connection?: PoolConnection,
  ): Promise<
    ...
    const [rows] = await db.query<RowDataPacket[]>(
      `
        ...
        WHERE m.is_active = 1
          ...
          AND LOWER(TRIM(m.scope_name)) = LOWER(TRIM(?))
          AND m.role IN ('WARD_SCOPE', 'DEPT_SCOPE')
          AND COALESCE(NULLIF(TRIM(m.citizen_id), ''), u.citizen_id) IS NOT NULL
      `,
      [scopeType, scopeName],
    );
  ...
  async findEmployeesByCitizenIds(
    citizenIds: string[],
    connection?: PoolConnection,
  ): Promise<
    {
  ...
  async findRequestCreateAuditMeta(
    requestId: number,
    connection?: PoolConnection,
  ): Promise<RowDataPacket | null> {
    const db = this.getDb(connection);
    const [rows] = await db.query<RowDataPacket[]>(
      `SELECT actor_id, actor_role, action_detail
       FROM audit_logs
       WHERE entity_type = 'request'
         AND entity_id = ?
         AND event_type = 'REQUEST_CREATE'
  ...
  async findEligibilityAttachments(
    eligibilityId: number,
    connection?: PoolConnection,
  ): Promise<EligibilityAttachmentEntity[]> {
    const db = this.getDb(connection);
    const [rows] = await db.query<RowDataPacket[]>(
      `SELECT * FROM eligibility_attachments WHERE eligibility_id = ? ORDER BY uploaded_at DESC`,
      [eligibilityId],
    );
    return rows as EligibilityAttachmentEntity[];
  }

  async findEligibilityAttachmentById(
    attachmentId: number,
    connection?: PoolConnection,
  ): Promise<EligibilityAttachmentEntity | null> {
    const db = this.getDb(connection);
    const [rows] = await db.query<RowDataPacket[]>(
      `SELECT * FROM eligibility_attachments WHERE attachment_id = ?`,
      [attachmentId],
    );
    return (rows[0] as EligibilityAttachmentEntity) || null;
  }
  ...
  async findEligibilityOcrPrecheck(
    eligibilityId: number,
    connection?: PoolConnection,
  ): Promise<RowDataPacket | null> {
    const db = this.getDb(connection);
    const [rows] = await db.query<RowDataPacket[]>(
      `SELECT * FROM eligibility_ocr_prechecks WHERE eligibility_id = ? LIMIT 1`,
      [eligibilityId],
    );
    return rows[0] ?? null;
  }

  async searchPersonnelOptions(
    search: string,
    limit = 20,
    connection?: PoolConnection,
  ): Promise<RowDataPacket[]> {
    ...
    const citizenDigits = searchText.replace(/\D/g, "");
    ...
    const [rows] = await db.query<RowDataPacket[]>(
      `
        SELECT
          ...
          ep.emp_type
        FROM users u
        JOIN emp_profiles ep ON ep.citizen_id = u.citizen_id
        WHERE u.is_active = 1
          AND (
            ? = ''
            OR u.citizen_id LIKE ?
            OR ep.first_name LIKE ?
            OR ep.last_name LIKE ?
            OR CONCAT(COALESCE(ep.title, ''), ep.first_name, ' ', ep.last_name) LIKE ?
            OR CONCAT(ep.first_name, ' ', ep.last_name) LIKE ?
            OR ep.position_name LIKE ?
          )
        ORDER BY ep.first_name ASC, ep.last_name ASC, u.id DESC
        LIMIT ?
      `,
      [
        searchText,
        citizenDigits ? `%${citizenDigits}%` : like,
        like,
        like,
        like,
        like,
  ...
  async deactivateActiveEligibilityByCitizenAndProfession(
    citizenId: string,
    professionCode: string,
    nextExpiryDate: string,
    exceptEligibilityId: number,
    connection: PoolConnection,
  ): Promise<number> {
    const [result] = await connection.execute<ResultSetHeader>(
      `
      UPDATE req_eligibility e
      JOIN cfg_payment_rates r ON r.rate_id = e.master_rate_id
         SET e.is_active = 0,
             e.expiry_date = CASE
               WHEN e.expiry_date IS NULL OR e.expiry_date > ? THEN ?
               ELSE e.expiry_date
             END
       WHERE e.citizen_id = ?
         AND r.profession_code = ?
         AND e.is_active = 1
  ...
  async setEligibilityActiveState(
    eligibilityId: number,
    isActive: boolean,
    ...
    await connection.execute(
      ...
      `,
      [isActive ? 1 : 0, expiryDate, eligibilityId],
    );
  }
  ...
  async insertEligibilityAttachment(
    data: Omit<EligibilityAttachmentEntity, "attachment_id" | "uploaded_at">,
    connection: PoolConnection,
  ): Promise<void> {
    await connection.execute(
  ...
  async upsertEligibilityOcrPrecheck(
    eligibilityId: number,
    data: {
      status: string;
      source?: string | null;
  ...
  async deleteEligibilityAttachmentById(
    attachmentId: number,
    connection?: PoolConnection,
  ): Promise<void> {
    const db = this.getDb(connection);
