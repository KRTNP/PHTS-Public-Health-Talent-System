...
describe("snapshot outbox service", () => {
  ...
  afterAll(() => {
    if (originalEnv.maxAttempts === undefined)
      delete process.env.SNAPSHOT_OUTBOX_MAX_ATTEMPTS;
    else process.env.SNAPSHOT_OUTBOX_MAX_ATTEMPTS = originalEnv.maxAttempts;
    if (originalEnv.retryBase === undefined)
      delete process.env.SNAPSHOT_OUTBOX_RETRY_BASE_SECONDS;
    else process.env.SNAPSHOT_OUTBOX_RETRY_BASE_SECONDS = originalEnv.retryBase;
    if (originalEnv.retryMax === undefined)
      delete process.env.SNAPSHOT_OUTBOX_RETRY_MAX_SECONDS;
    else process.env.SNAPSHOT_OUTBOX_RETRY_MAX_SECONDS = originalEnv.retryMax;
    if (originalEnv.timeout === undefined)
      delete process.env.SNAPSHOT_OUTBOX_PROCESSING_TIMEOUT_SECONDS;
    else
  ...
  test("reclaims stuck processing rows before reading pending work", async () => {
    const { SnapshotRepository } =
      await import("@/modules/snapshot/repositories/snapshot.repository.js");
    const conn = {
    ...
    (
      SnapshotRepository.reclaimStuckProcessing as jest.Mock
    ).mockResolvedValueOnce(2);
    (
      SnapshotRepository.findOutboxBatchForUpdate as jest.Mock
    ).mockResolvedValueOnce([]);

    const { processSnapshotOutboxBatch } =
      await import("@/modules/snapshot/services/snapshot.service.js");
    ...
    expect(SnapshotRepository.findOutboxBatchForUpdate).toHaveBeenCalledWith(
      20,
      8,
      conn,
    );
  ...
  test("marks failed rows with retry policy when snapshot generation fails", async () => {
    ...
    const { emitAuditEvent } =
      await import("@/modules/audit/services/audit.service.js");
    const conn = {
      beginTransaction: jest.fn().mockResolvedValue(undefined),
      commit: jest.fn().mockResolvedValue(undefined),
      rollback: jest.fn().mockResolvedValue(undefined),
    ...
    (
      SnapshotRepository.findOutboxBatchForUpdate as jest.Mock
    ).mockResolvedValueOnce([{ outbox_id: 44, period_id: 9, requested_by: 7 }]);
    (
      SnapshotRepository.findPeriodByIdForUpdate as jest.Mock
    ).mockResolvedValueOnce({
      period_id: 9,
      period_month: 2,
      period_year: 2026,
    ...
    (
      SnapshotRepository.findPayoutsForSnapshot as jest.Mock
    ...
    expect(SnapshotRepository.markOutboxProcessing).toHaveBeenCalledWith(
      44,
      conn,
    );
    ...
    expect(SnapshotRepository.setPeriodSnapshotFailed).toHaveBeenCalledWith(
      9,
      conn,
    );
  ...
  test("sums string total_payable values as numbers when creating snapshots", async () => {
    ...
    const { processSnapshotOutboxBatch, SnapshotType } =
      await import("@/modules/snapshot/services/snapshot.service.js");
    const result = await processSnapshotOutboxBatch(10);
