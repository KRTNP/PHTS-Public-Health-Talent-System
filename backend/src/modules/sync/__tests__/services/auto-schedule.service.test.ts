...
describe("sync auto schedule service", () => {
  ...
  test("uses default daily schedule when no config is set", async () => {
    const { getSyncAutoScheduleConfig } =
      await import("@/modules/sync/services/sync-auto-schedule.service.js");
  ...
  test("persists interval schedule and returns normalized values", async () => {
    const { setSyncAutoScheduleConfig, getSyncAutoScheduleConfig } =
      await import("@/modules/sync/services/sync-auto-schedule.service.js");
  ...
  test("daily mode runs once after scheduled minute and dedupes same day", async () => {
    const { setSyncAutoScheduleConfig, shouldRunAutoSync } =
      await import("@/modules/sync/services/sync-auto-schedule.service.js");
    ...
    await expect(
      shouldRunAutoSync(new Date("2026-03-01T03:09:00+07:00")),
    ).resolves.toBe(false);
    await expect(
      shouldRunAutoSync(new Date("2026-03-01T03:10:00+07:00")),
    ).resolves.toBe(true);
    await expect(
      shouldRunAutoSync(new Date("2026-03-01T05:45:00+07:00")),
    ).resolves.toBe(false);
    await expect(
      shouldRunAutoSync(new Date("2026-03-02T03:10:00+07:00")),
    ).resolves.toBe(true);
  });
  ...
  test("interval mode runs once per interval bucket", async () => {
    ...
    await expect(
      shouldRunAutoSync(new Date("2026-03-01T03:00:00+07:00")),
    ).resolves.toBe(true);
    await expect(
      shouldRunAutoSync(new Date("2026-03-01T03:29:00+07:00")),
    ).resolves.toBe(false);
    await expect(
      shouldRunAutoSync(new Date("2026-03-01T03:30:00+07:00")),
    ).resolves.toBe(true);
  });
});
