...
jest.mock(
  "@/modules/identity/services/identity-role-policy.service.js",
  () => ({
    __esModule: true,
...
jest.mock(
  "@/modules/sync/repositories/transform-monitor.repository.js",
  () => ({
    __esModule: true,
...
jest.mock("@/modules/sync/services/sync-status.service.js", () => ({
  __esModule: true,
  getSyncRuntimeStatus: jest.fn(async () => ({
    isSyncing: false,
    lastResult: null,
  })),
}));
...
jest.mock(
  "@/modules/workforce-compliance/services/immediate-rules.service.js",
  () => ({
    __esModule: true,
...
jest.mock("@/modules/sync/services/shared/sync-lock.service.js", () => ({
  ...
  getLastSyncStatus: jest.fn(async () => ({
    isSyncing: false,
    lastResult: null,
  })),
...
jest.mock("@/modules/sync/services/shared/sync-db-helpers.service.js", () => ({
  ...
  resolveSupportStaffColumnFlags: jest.fn(async () => ({
    hasLevelColumn: true,
  })),
...
jest.mock(
  "@/modules/sync/repositories/sync-query-builders.repository.js",
  () => ({
    __esModule: true,
...
describe("SyncService execution policy", () => {
  ...
  test("performUserSync aborts when another sync already holds the lock", async () => {
    ...
    await expect(
      mod.SyncService.performUserSync(42, { triggeredBy: 7 }),
  ...
  test("executePipelineSync keeps a committed batch successful when cache write fails after commit", async () => {
    const mod = await loadModule();
    const executePipelineSync = (
      mod.SyncService as any
    ).executePipelineSync.bind(mod.SyncService);
  ...
  test("pipeline actions do not receive legacy normalizeLeaveRow dependency", async () => {
    const mod = await loadModule();
    const domainMod =
      await import("@/modules/sync/services/domain/sync-domain.service.js");
    ...
    const leaveSyncDeps = (domainMod.syncLeaves as jest.Mock).mock
      .calls[0]?.[2];
