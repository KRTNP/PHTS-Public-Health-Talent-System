...
const loadModule = async () =>
  import("../../services/domain/sync-scope.service.js");
...
describe("buildScopesFromSpecialPosition", () => {
  test("parses head mission scopes as dept scopes", async () => {
    ...
    expect(result).toEqual({
      wardScopes: [],
      deptScopes: ["ภารกิจด้านบริการปฐมภูมิ", "ภารกิจด้านทุติยภูมิและตติยภูมิ"],
    });
  });
  ...
  test("parses head prefixes with optional separator/spacing and normalizes dept label", async () => {
    ...
    const result = build?.(
      "หัวหน้ากลุ่มงานการเงิน; หัวหน้าตึก/หัวหน้างาน - ICU Neuro.",
    );
  ...
  test("does not create scopes when profile special_position is empty", async () => {
    ...
    const deps = {
      citizenIdJoinBinary: jest
        .fn()
        .mockReturnValue("u.citizen_id = e.citizen_id"),
  ...
  test("creates scope mappings even when current user role is USER", async () => {
        ...
        [
          {
            ...
            citizen_id: "3102200342989",
            special_position:
              "หัวหน้ากลุ่มงาน-กลุ่มงานเภสัชกรรม,หัวหน้าตึก-ICU",
    ...
    expect(deps.disableScopeMappings).toHaveBeenCalledWith(
      43815,
      DB_HEAD_SCOPE_ROLE_WARD,
      conn,
    );
