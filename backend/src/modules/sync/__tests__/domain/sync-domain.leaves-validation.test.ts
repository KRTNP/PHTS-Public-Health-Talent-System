...
import {
  createLeaveSyncDeps,
  createLeaveSyncStats,
} from "./sync-domain.leaves-validation.test-helpers.js";
...
describe("sync-domain leave validation guards", () => {
  test("syncLeaves looks up existing rows only for incoming ref_id values", async () => {
    ...
    expect((conn.query as jest.Mock).mock.calls[1]?.[0]).toContain(
      "WHERE ref_id IN (?)",
    );
    expect((conn.query as jest.Mock).mock.calls[1]?.[0]).toContain(
      "leave_type",
  ...
  test("syncLeaves removes synced leave rows whose citizen is no longer in emp_profiles", async () => {
    ...
    await syncLeaves(
      conn,
      stats,
      createLeaveSyncDeps({ isChanged: () => false }),
    );
    ...
    expect((conn.execute as jest.Mock).mock.calls.at(-1)?.[0]).toContain(
      "ref_id IS NOT NULL",
    );
    expect((conn.execute as jest.Mock).mock.calls.at(-1)?.[0]).toContain(
      "FROM emp_profiles e",
