const loadModule = async () =>
  import("../../repositories/sync-query-builders.repository.js");
...
describe("sync query builders repository", () => {
  ...
  test("keeps view queries scoped to HRMS/profile sources without users joins", async () => {
    ...
    expect(leavesSql).toContain("EXISTS (");
    expect(leavesSql).toContain("SELECT 1 FROM emp_profiles e");
  ...
  test("builds single leave view query scoped to emp_profiles only", async () => {
    const mod = await loadModule();

    const sql = (mod as any).buildSingleLeaveViewQuery("citizen_predicate");
