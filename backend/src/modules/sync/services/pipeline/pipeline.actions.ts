...
type PipelineActionDeps = {
  ...
  resolveSupportStaffColumnFlags: (
    conn: PipelineContext["conn"],
  ) => Promise<{ hasLevelColumn: boolean }>;
...
export const createSyncPipelineActions = (
  ...
  syncEmployeeProfiles: async (ctx: PipelineContext) => {
    if (mode === "USER") {
      ...
      await deps.upsertSingleEmployeeProfile(
        ctx.conn,
        ctx.citizenId,
        userIdMap,
        ctx.stats,
  ...
  syncSupportStaff: async (ctx: PipelineContext) => {
    if (mode === "USER") {
      if (!ctx.citizenId) return { skipped: true };
      await deps.upsertSingleSupportEmployee(
        ctx.conn,
        ctx.citizenId,
        ctx.stats,
        {
  ...
  assignRoles: async (ctx: PipelineContext) => {
    if (mode === "USER") {
      ...
      await deps.assignRoleForSingleUser(
        ctx.conn,
        userContext.dbUser,
        ctx.citizenId,
        ctx.stats,
    ...
    const roleResult = await deps.assignRoles(ctx.conn);
