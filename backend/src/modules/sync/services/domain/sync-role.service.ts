...
import { toDbHeadScopeRole } from "@/shared/utils/head-scope-category.js";
...
export const assignRoleForSingleUser = async (
  ...
  try {
    ...
    if (!hrRow) {
      if (
        deps.roleAssignmentService.AUTO_ASSIGNABLE_ROLES.has(
    ...
    if (derivedRole === "WARD_SCOPE" || derivedRole === "DEPT_SCOPE") {
      const [scopeRows] = await conn.query<RowDataPacket[]>(
        `
          ...
          LIMIT 1
        `,
        [toDbHeadScopeRole(derivedRole), citizenId],
      );
