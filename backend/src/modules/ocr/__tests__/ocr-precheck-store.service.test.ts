...
describe("ocr precheck store service", () => {
  ...
  it("loads existing OCR results for request scope", async () => {
    ...
    const { getExistingOcrResults } =
      await import("@/modules/ocr/services/ocr-precheck-store.service.js");
  ...
  it("loads existing OCR results for eligibility scope from results_json", async () => {
    (
      requestRepository.findEligibilityOcrPrecheck as jest.Mock
    ).mockResolvedValue({
      results_json: JSON.stringify([
        { name: "memo.pdf", ok: true, markdown: "text" },
      ]),
    });
