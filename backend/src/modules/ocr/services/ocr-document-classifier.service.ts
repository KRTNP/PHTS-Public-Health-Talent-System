...
export const classifyOcrDocument = (
  item: OcrBatchResultItem,
): OcrDocumentKind => {
  ...
  const memoSignalCount = [
    lines.some((line) =>
      /(?:บันทึกข้อความ|บนทกขอความ|บันทึกขอความ)/.test(line),
    ),
  ...
  if (
    /คำสั่ง/.test(markdown) &&
    /(มอบหมาย|รับผิดชอบ|ปฏิบัติงาน)/.test(markdown)
  ) {
  ...
  if (
    [
      ...
      lines.some(
        (line) =>
          /(ประกอบวิชา|วิชาจีพ|วิชาชีพ)/.test(line) &&
          /(พยาบาล|ผยาบาล|แยายาล|เภสัช)/.test(line),
      ),
      lines.some((line) =>
        /(ต่ออายุครั้งที่|ตออายุตรงท|ตอยอายุตรงท)/.test(line),
      ),
      lines.some((line) => /(หมดอายุ|หผดอายุ|ทมดอาย|มดอายุ|หผดอาย)/.test(line)),
      lines.some((line) =>
        /(ออกใบอนุญาตนี้ให้แก่|ออกใบอนุญาตนี้ให้แก|จอกใบอนุญาต|ลอกใบอนญาต)/.test(
          line,
        ),
      ),
