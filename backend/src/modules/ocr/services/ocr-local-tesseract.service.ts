...
const getTesseractLang = (): string =>
  (process.env.OCR_TESSERACT_LANG || DEFAULT_TESSERACT_LANG).trim() ||
  DEFAULT_TESSERACT_LANG;

const getTesseractOem = (): string =>
  getEnvNumberString(
    process.env.OCR_TESSERACT_OEM,
...
const getPdfRenderDpi = (): string =>
  getEnvNumberString(
    process.env.OCR_TESSERACT_PDF_DPI,
...
const getThreadLimit = (): string =>
  getEnvNumberString(
    process.env.OCR_TESSERACT_THREAD_LIMIT,
    DEFAULT_TESSERACT_THREAD_LIMIT,
    1,
    16,
  );
...
const getPreprocessMode = (): string =>
  (process.env.OCR_TESSERACT_PREPROCESS || DEFAULT_TESSERACT_PREPROCESS)
    .trim()
    .toLowerCase();
