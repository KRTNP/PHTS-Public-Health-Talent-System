...
describe('Payroll Integration: Retroactive Scenarios', () => {
  ...
  test('TC-PAY-02: Retroactive Logic (Fix Verification)', async () => {
    ...
    await pool.query(
      `INSERT INTO pay_periods (period_year, period_month, status) VALUES (2024, 1, 'CLOSED')`,
    );
    const [periodRes]: any = await pool.query(`SELECT period_id FROM pay_periods WHERE period_month = 1`);
  ...
  test('TC-PAY-08: Retroactive Deduction (Clawback/Overpayment)', async () => {
    ...
    const [periodRes]: any = await pool.query(
      `SELECT period_id FROM pay_periods WHERE period_month = 1 AND period_year = 2024`,
    );
    const periodIdJan = periodRes[0].period_id;
  ...
  test('TC-REAL-01: Split Month & Retroactive (Case Study: Doctor Split Periods)', async () => {
    ...
    await pool.query(
      `INSERT INTO pay_periods (period_year, period_month, status) VALUES (2024, 6, 'CLOSED'), (2024, 7, 'CLOSED')`,
    );
  ...
  test('TC-BRUTAL-01: The Double Retro Trap (Prevent Duplicate Payments)', async () => {
    ...
    await pool.query(`INSERT INTO pay_periods (period_year, period_month, status) VALUES (2024, 1, 'CLOSED')`);
    const [pJan]: any = await pool.query(`SELECT period_id FROM pay_periods WHERE period_month = 1`);
    await pool.query(
      `
      INSERT INTO pay_results (period_id, citizen_id, master_rate_id, pts_rate_snapshot, calculated_amount, total_payable)
    ...
    const [pFeb]: any = await pool.query(`SELECT period_id FROM pay_periods WHERE period_month = 2`);
