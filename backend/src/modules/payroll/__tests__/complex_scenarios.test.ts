...
describe('Payroll Integration: Advanced Complex Scenarios', () => {
  ...
  test('TC-RETRO-01: Retroactive Upgrade (underpaid last month -> top up this month)', async () => {
    ...
    await pool.query(
      `INSERT INTO pts_employee_eligibility (citizen_id, master_rate_id, effective_date, is_active) VALUES (?, ?, '2024-07-01', 1)`,
      [cid, r10k[0].rate_id],
    );
    ...
    await pool.query(`INSERT INTO pts_periods (period_month, period_year, status) VALUES (7, 2024, 'CLOSED')`);
    const [periodJuly]: any[] = await pool.query(`SELECT period_id FROM pts_periods WHERE period_month=7`);

    await pool.query(
      `INSERT INTO pts_payouts (period_id, citizen_id, calculated_amount, total_payable) VALUES (?, ?, 5000, 5000)`,
  ...
  test('TC-RETRO-02: Clawback (overpaid last month -> deduct this month)', async () => {
    ...
    await pool.query(
      `INSERT INTO pts_employee_licenses (citizen_id, valid_from, valid_until, status) VALUES (?, '2020-01-01', '2024-07-15', 'ACTIVE')`,
      [cid],
    );
    ...
    const [periodJuly]: any[] = await pool.query(`SELECT period_id FROM pts_periods WHERE period_month=7`);
    await pool.query(
      `INSERT INTO pts_payouts (period_id, citizen_id, calculated_amount, total_payable) VALUES (?, ?, 10000, 10000)`,
      [periodJuly[0].period_id, cid],
    );
