...
import {
  buildWeekdayOneDayLeaves,
  makeBaseInput,
} from "./calculator.license.shared.js";
...
test("missing start_work_date raises blocker check and does not fallback to first_entry_date", async () => {
  ...
  expect(result.totalDeductionDays).toBe(0);
  expect(result.checks?.some((c) => c.code === "MISSING_START_WORK_DATE")).toBe(
    true,
  );
  expect(
    result.checks?.some(
...
test("does not extend deduction for missing return report and raises warning instead", async () => {
  ...
  expect(result.totalDeductionDays).toBe(0);
  expect(result.checks?.some((c) => c.code === "PENDING_RETURN_REPORT")).toBe(
    true,
  );
  expect(
    result.checks?.some(
