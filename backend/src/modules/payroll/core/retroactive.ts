...
const calculateRetroForPeriod = async (
  ...
  const recalculated = await calculateMonthly(citizenId, targetYear, targetMonth, connection);
...
export async function calculateRetroactive(
  ...
  for (let i = 1; i <= lookBackMonths; i++) {
    const { targetMonth, targetYear } = shiftMonth(currentYear, currentMonth, i);
